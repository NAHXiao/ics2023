...
word_t mmio_read(paddr_t addr, int len) {
  ...
  word_t result = 0;
  IFDEF(CONFIG_DTRACE, MyLog("Dr name:%10s addr:0x%08x ", map->name,addr));
  result = map_read(addr, len, map);
  IFDEF(CONFIG_DTRACE, MyLog("data:0x%08x\n", result));
  return result;
}

void mmio_write(paddr_t addr, int len, word_t data) {
  IOMap *map = fetch_mmio_map(addr);
  IFDEF(CONFIG_DTRACE, MyLog("Dw name:%10s addr:0x%08x data:0x%08x\n", map->name,addr,data));
